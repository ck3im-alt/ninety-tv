...
The account website can queue add/rename/refresh/delete commands for a connected TV. The TV sends only playlist metadata and applies changes through its existing importer/library while idle. Provider credentials are never included in snapshots or error acknowledgements. Updated API migrations through `1755002100000` must be deployed first. One-TV/two-TV access comes from the authoritative API; a TV outside its paid plan gets the existing purchase QR flow directed to subscription management. Founding Member lifetime access is preserved.
