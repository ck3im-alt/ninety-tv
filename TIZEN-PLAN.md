...
Nåværende EPG-markeder i `ninety-api` (alle via ekte XMLTV-guider, ikke stub-data): NO, SE, DK, GB, FR, TR, DE, PT, **ES, IT** (de to siste lagt til i Phase 2C, 2026-08-20, live-validert mot produksjon — se prosjektminner for full rapport). Strømrangering (`features/eventDetails/rankStreamQuality.ts`) er bevisst fortsatt ren metadata-basert — ingen probing av faktiske strømmer i Event Details/rangering, per den fysiske Tizen-kvalifiseringens krav om null IPTV-tilkoblinger før avspilling faktisk starter.

Kjente gjenværende resolver-hull (ikke løst, dokumentert som reelle grenser, ikke bugs): flere UEFA Europa League-kvalifiseringskamper og et par Serie A-kamper i det nåværende valideringsvinduet har ingen kringkastingsdekning i noen av de nå ni ingesterte EPG-kildene ennå (bekreftet via faktisk kandidat-inspeksjon, ikke antatt) — UNKNOWN er riktig oppførsel her, ikke en feil. Neste reelle steg: fortsett kalibrering av kanal-matching per marked etter hvert som flere hendelser ruller inn i hvert markeds EPG-horisont (se prosjektminner for status per land).

**⚠️ Neste påkrevde spiller-milepæl før AVPlay vurderes: reell hardware-kvalifisering.** `src/core/player/htmlVideoPlayer.ts` (HTML5 `<video>` + hls.js/mpegts.js) er bevisst ikke testet på fysisk Samsung TV-maskinvare ennå — kun i nettleser/dev. Før noe AVPlay-arbeid (native Tizen-spiller-API) starter, må hele sjekklisten i [`docs/TIZEN-HARDWARE-QUALIFICATION.md`](docs/TIZEN-HARDWARE-QUALIFICATION.md) kjøres på minst én fysisk Samsung TV (helst én eldre 2018-2020-modell med begrenset RAM + én nyere modell). Sjekklisten dekker install/launch, fjernkontroll, avspillingsformater (MPEG-TS/HLS/H.264/HEVC/AAC/AC3/E-AC3), spiller-utholdenhet, leverandør-tilkoblingsgrenser, nettverksavbrudd, app-livssyklus og minne/store spillelister — og ender i en PASS/CONDITIONAL/FAIL-beslutning. **AVPlay skal kun bygges hvis resultatet er FAIL, med konkret, reproduserbar bevis** (spesifikk test-ID, TV-modell/firmware, faktisk feil) — ikke generell mistanke om at native spiller "sikkert er bedre".
